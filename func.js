...
(function() {
  ...
  globalThis.runEnhancement = async () => {
    ...
    for (const _ of [...Array(4)]) {
      ...
      const targets = [
        ...document.querySelectorAll('.run-output, .run-output *, .react-code-text, code, [class*="log-viewer"], .yaml-editor, .CodeMirror-lines, pre:not(:has(*)),pre :not(.non-alpha,[class*="highlight"],:has(*)), html:not([data-origin*="jenkins"]) a, time, td'),
