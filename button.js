...
(function() {
  ...
  const runloop = async () => {
    if (loopStarted) return; // STRICT GUARD: Prevent duplicate execution loops
    loopStarted = true;

    const btn = document.createElement("button");
    btn.innerHTML = "🟢 Glow";
    btn.style = "position:fixed; bottom:10px; right:10px; z-index:10000; padding:10px; background:#222; color:#00ff00; border:1px solid #00ff00; cursor:pointer; font-weight:bold; border-radius: 5px;";

    const updateBtnStyle = () => {
      btn.style.filter = globalStore.glow ? 'none' : 'grayscale(1)';
    };

    btn.onclick = () => {
      globalStore.glow = !globalStore.glow;
      updateBtnStyle();
    };
    ...
    while (true) {
      ...
      if (
        ...
        if (Date.now() - globalThis.lastRun > interval) {
          globalThis.lastRun = Date.now();
          await runEnhancement();
        }
      }
  ...
  // Safe orchestrator for single execution kickoff
