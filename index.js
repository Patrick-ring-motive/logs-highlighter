...
(function () {
  ...
  const isFocus = (node) => {
    if (matchesNode(node, focusCSS)) {
      if (node?.dataset?.touched ?? node?.parentElement?.dataset?.touched)
        return true;
  ...
  const loadPrism = () =>
    new Promise((resolve) => {
      ...
      if (!/github\.com/.test(location.href))
    ...
    nodes.forEach((textNode) => {
      ...
      if (regex.test(text)) {
        ...
        while ((match = regex.exec(text)) !== null) {
          ...
          hasChanges = true;
          if (
            !textNode.parentElement.matches(
              ':has(.pipeline-new-node),td,td *,[style*="relative"] *',
            ) &&
      ...
      if (
        numRegex.test(text) &&
        !/highlight-nums|number/.test(textNode?.parentElement?.className)
      ) {
        //  (textNode.parentElement?.style??{}).backgroundColor = bkColor;
        const fragment = document.createDocumentFragment();
        let lastIndex = 0;
  ...
  let counter = 0;
  const interval = 500;
  globalThis.lastRun = Date.now();
  runEnhancement();
  const runloop = async () => {
    const btn = document.createElement("button");
    btn.innerHTML = "🟢 Glow";
    btn.style =
      "position:fixed; top:10px; right:10px; z-index:10000; padding:10px; background:#222; color:#00ff00; border:1px solid #00ff00; cursor:pointer; font-weight:bold; border-radius: 5px;";
  ...
  document.addEventListener("load", runloop);
  if (document.readyState == "complete") runloop();
})();
